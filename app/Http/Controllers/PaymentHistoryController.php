...
class PaymentHistoryController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $total_tax = $queryBuilder->where('payment_status', 1)->sum('point_subscription_history.tax');
        $total_amount = $queryBuilder->where('payment_status', 1)->sum('point_subscription_history.amount');
    ...
    public function export()
    {
        ...
        foreach ($paymentList as &$item) {
            ...
            $input['売上'] = $this->convertShijis($item['payment_status'] == 0 ? 0 : $item['amount']);
            $input['消費税'] = $this->convertShijis($item['payment_status'] == 0 ? 0 : $item['tax']);
