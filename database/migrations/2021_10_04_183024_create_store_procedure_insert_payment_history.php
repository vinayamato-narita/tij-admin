...
BEGIN
    ...
    SELECT COALESCE(parent.course_code, '') 
     FROM lms_project_courses lpc
     LEFT JOIN lms_project_courses parent ON lpc.parent_id = parent.id
     WHERE lpc.project_id = _project_id
     AND lpc.id = _project_course_id
     LIMIT 1
    );
    ...
    SET _first = (
    SELECT COUNT(*) FROM lms_project_course_students lpcs
    LEFT JOIN point_subscription_histories psh ON lpcs.`point_subscription_id` = psh.`id`
    ...
    FROM lms_project_course_students
    JOIN students ON students.id = lms_project_course_students.student_id
    WHERE 
    ...
    INSERT INTO lms_project_course_students(
    project_course_id,
    project_id,
    course_id,
    student_id,
    ...
    UPDATE lms_project_students 
    SET buy_course_flag = (SELECT buy_course_continue FROM lms_projects WHERE lms_projects.id = _project_id) 
...
BEGIN
    IF _cancel_history_save_flag = 1 THEN
        INSERT INTO lesson_cancel_histories (
            `student_id`,
            `teacher_id`,
            `lesson_date`,
            `lesson_starttime`,
        ...
        SELECT 
                ...
                FROM 
                    lesson_histories lh
                    join lesson_schedules ls on lh.lesson_schedule_id = ls.id
                    JOIN student_point_histories sph ON sph.lesson_schedule_id = ls.id
        ...
        DELETE FROM student_point_histories WHERE point_subscription_id = _point_subscription_history_id;
        
        DELETE FROM point_subscription_histories WHERE id = _point_subscription_history_id;
        
        DELETE FROM lms_project_course_students WHERE point_subscription_id = _point_subscription_history_id;
END";
    ...
    SELECT
        ...
        ,IF(is_set_course = 1,
          (SELECT SUM(child.amount)
              FROM course_set_course sc JOIN courses child ON sc.course_id = child.course_id 
...
END";
    ...
    IN _course_free_id INT(11))
BEGIN
  SELECT 
    lpc.id as project_course_id,
    lpc.project_id,
    lpc.course_type,
    lpc.parent_id,
    c.course_id,
    ...
    (SELECT SUM(lpc1.price)
         FROM lms_project_courses lpc1 
         WHERE lpc1.set_course_id = c.course_id AND lpc1.parent_id = lpc.id), 
    lpc.price
    ) AS price,
    c.is_show
  FROM
    lms_project_students lps
      LEFT JOIN lms_project_courses lpc ON lps.project_id = lpc.project_id
    LEFT JOIN courses c ON lpc.course_id = c.course_id
     LEFT JOIN lms_projects lp ON lp.id = lpc.project_id
   WHERE 
   ...
   ORDER BY lps.`created_at` DESC, lpc.`order_number`, lpc.`price`
  ;
END";
...
VALUES (_order_id,
        ...
        _course_id,
        _product_code,
        (SELECT campaign_code FROM courses WHERE course_id = _course_id),
        _order_date,
        _order_status,
        _order_ip,
        _gmo_error_code,
...
BEGIN
    ...
    SELECT lc.favourite_code,lc.legal_code INTO _default_customer_code, _default_corporation_code FROM lms_companies lc LEFT JOIN lms_project_courses lp ON lc.id = lp.company_id WHERE lp.id = _project_course_id;
    
    IF _project_course_id > 0 THEN
       SET @project_id = (SELECT project_id FROM lms_project_courses WHERE lms_project_courses.id = _project_course_id);
       SELECT corporation_flag INTO _corporation_flag FROM lms_projects lp WHERE lp.id = @project_id;
    ...
    SET @point_expire_day = (SELECT point_expire_day FROM courses WHERE courses.course_id=_course_id);
    IF _project_course_id > 0 THEn -- LMS user
    SET @score_grace_period = (SELECT COALESCE(score_grace_period, 0) FROM lms_project_courses WHERE lms_project_courses.id = _project_course_id);   
 ...
 -- insert order
    SET @campaign_code = (SELECT campaign_code FROM courses WHERE courses.course_id = IF(_set_course_id = 0, _course_id, _set_course_id) );
    SET @orderExist = (SELECT COUNT(order_id) FROM `orders` where orders.order_id = _order_id);
...
BEGIN
    ...
    SET @course_begin_month = IF(_course_begin_month <> '' , _course_begin_month,(SELECT course_begin_month FROM lms_project_course_students WHERE student_id = _student_id AND point_subscription_id = _payment_id));
        ...
        UPDATE point_subscription_histories
        SET
        ...
        UPDATE `student_point_histories`
        SET 
        ...
        UPDATE lms_project_course_students
        SET 
        start_date = _start_date,
...
BEGIN
SET @point_count = (SELECT SUM(point_count) FROM student_point_histories where student_id = _student_id AND course_id = 1);
IF @point_count > 0 THEn
   INSERT INTO student_point_histories(student_id,pay_date,pay_description,pay_type,point_count,expire_date,lesson_schedule_id,course_id,point_subscription_id)
              SELECT
                _student_id
                , NOW()
                ,'失効'
                ...
                ,1
            ,point_subscription_id
            FROM student_point_histories WHERE student_id = _student_id AND course_id = 1 LIMIT 1;
