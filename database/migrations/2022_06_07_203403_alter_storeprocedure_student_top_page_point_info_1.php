...
class AlterStoreprocedureStudentTopPagePointInfo1 extends Migration
{
    /**
     * Run the migrations.
     *
...
BEGIN
            ...
            SET @time_diff_to = COALESCE((SELECT diff_time * 60 FROM timezone WHERE timezone_id = @timezone_id),9 * 60);
               ...
               INNER JOIN point_subscription_history psh ON psh.point_subscription_history_id = p.point_subscription_id and psh.payment_status = 1 
               WHERE p.student_id = _student_id
                AND p.paid_status = 1
                AND TIMESTAMP(expire_date) >= TIMESTAMP(NOW())
                AND psh.payment_status = 1
                
               GROUP BY p.point_subscription_id
               ;
            END
        ";
