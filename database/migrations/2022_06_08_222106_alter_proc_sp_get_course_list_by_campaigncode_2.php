...
class AlterProcSpGetCourseListByCampaigncode2 extends Migration
{
    /**
     * Run the migrations.
     *
...
BEGIN
            ...
            SELECT course.course_id,
                ...
                DATE_FORMAT(course.decide_date, '%Y/%m/%d %H:%i') as decide_date_show,
                                DATE_FORMAT(DATE_ADD(DATE_ADD(course.decide_date,INTERVAL -@time_diff_from MINUTE),INTERVAL @time_diff_to MINUTE),'%Y/%m/%d %H:%i')
                        AS decide_date_show_timezone,
                ca.category_id,
            ...
            WHERE CASE WHEN _category_id <> 0 THEN ca.category_id = _category_id ELSE 1 = 1 END
               AND CASE WHEN (_course_type <> 999 AND _course_type <> 98) THEN course.course_type = _course_type ELSE 1 = 1 END
               AND CASE WHEN _course_type = 98 THEN (cam.course_campaign_id IS NOT NULL) ELSE 1 = 1 END
