                        ...
                        <div class="col-md-12">
                            <div class="card" style="margin: 10px; min-height: 600px;">
                                <div class="card-header">
                                    <h5 class="title-page">IMPORT FORM(法人・セットコース）</h5>
                                </div>
                                ...
                                <div class="error-des">
                                    ...
                                    <table>
                                        ...
                                        <tr>
                                            <td class="error-td-class">3.7 セットコードチェック</td>
                                    ...
                                    <div style="overflow: scroll;">
                                        <table class='table table-bordered resize' style="margin: 10px!important;" v-if="csv.length">
                                            ...
                                            <tr v-for="(row, keyRow) in csv" v-if="keyRow != 0">
                                                <th v-for="(value, key) in csv[0]" class="error error-common" v-if="getConditionHeader(row, keyRow)"> 
                                                    {{ row[key] }}
                                                </th>
                                                <td v-for="(value, key) in row" :class="getTdClass(row, key)" v-if="!isNaN(key) && getConditionTd(row, keyRow)">
...
export default {
    ...
    props: ["urlAction", "urlImportCsv", "studentId", "courseId", "courseBeginMonth", "orderDate", "startDate", "beginDate", "expireDate", "managementNumber"],
    mounted() {},
    methods: {
        save() {
            let that = this;
        ...
        getConditionHeader(row, keyRow) {
            return keyRow != 0 && row['error_list'] != undefined && row['error_list']['class'] != undefined && row['error_list']['class'] == 'error-common';
        },
        getConditionTd(row, keyRow) {
            return keyRow != 0 && (row['error_list'] == undefined || row['error_list']['class'] == undefined || row['error_list']['class'] != 'error-common');
        }
    }
};
