        ...
        <div class="container-fluid">
            <div class="page-heading">
                ...
                <div class="pull-right mrb-5">
                    <a href="{{ route('file.create') }}" class="btn btn-primary pull-right"
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="row mb-2">
                                    ...
                                    <div class="col-md-10">
                                        <input-search :page-limit="{{ $pageLimit }}" :url="{{ json_encode(route('file.index')) }}" :data-query="{{json_encode(!empty($request) ? $request->all() : new stdClass)}}"></input-search>
                                    </div>
                                    
                                </div>
                                @if(!$files->isEmpty())
                                    {{ $files->appends(SearchQueryComponent::alterQuery($request))->links('pagination.paginate') }}
                                        ...
                                        <table class="table table-responsive-sm table-striped border">
                                            ...
                                            <tbody>
                                                @foreach ($files as $index => $file)
                                                    <tr class="eachRow">
                                                        <td class="text-left status">{{ $file->file_id }}</td>
                                                        <td class="text-left">{{ $file->file_code }}</td>
                                                        <td class="text-left">{{ $file->file_display_name }}</td>
                                                        <td class="text-left">{{ $file->file_description }}</td>
                                                        <td>
                                                            <div class="btn-group">
